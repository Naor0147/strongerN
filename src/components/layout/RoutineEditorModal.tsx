...
const RoutineEditorModal: React.FC<RoutineEditorModalProps> = ({
  ...
  // Memoized O(1) lookup Map for exercises library
  ...
  // Fast single-pass helper to derive RoutineExercise array
  const deriveInitialExercises = useCallback((
    details?: any[],
    names?: string[]
  ): RoutineExercise[] => {
    const timestamp = Date.now();
    if (details && details.length > 0) {
      return details.map((ex, exIdx) => {
        ...
        const sets = (Array.isArray(ex.sets) && ex.sets.length > 0)
          ...
          : [
              { id: `s-${exIdx}-0-${timestamp}`, weight: '0', reps: '0', category: 'S' as const, isUnilateral },
              { id: `s-${exIdx}-1-${timestamp}`, weight: '0', reps: '0', category: 'S' as const, isUnilateral },
              { id: `s-${exIdx}-2-${timestamp}`, weight: '0', reps: '0', category: 'S' as const, isUnilateral },
            ];
        return {
          id: `ex-${exIdx}-${timestamp}`,
          name: ex.name,
    ...
    return (names || []).map((name, idx) => {
      const libEx = exerciseMap.get(name.toLowerCase());
      const isUnilateral = libEx?.isUnilateral || false;
      return {
        id: `ex-${idx}-${timestamp}`,
        name,
        notes: libEx?.notes || '',
        sets: [
          { id: `s-${idx}-0-${timestamp}`, weight: '0', reps: '0', category: 'S' as const, isUnilateral },
          { id: `s-${idx}-1-${timestamp}`, weight: '0', reps: '0', category: 'S' as const, isUnilateral },
          { id: `s-${idx}-2-${timestamp}`, weight: '0', reps: '0', category: 'S' as const, isUnilateral },
        ],
      };
    });
  }, [exerciseMap]);
  ...
  if (prevVisibleRef.current !== visible) {
    ...
    if (visible) {
      ...
      // Compute initial exercises derived entirely from props & exerciseMap
      const initialComputed = deriveInitialExercises(initialExercisesDetails, initialExercises);
  ...
  const addSet = useCallback((exIdx: number) => {
    ...
    setEditorExercises(prev => {
      ...
      const libEx = exerciseMap.get(exName.toLowerCase());
                        ...
                        <>
                          ...
                          <Pressable
                            ...
                            onPress={() => {
                              ...
                              const libEx = exerciseMap.get(currentEx.name.toLowerCase());
      ...
      >
        <Pressable style={edStyles.sheetBackdrop} onPress={() => setIsRoutineSettingsVisible(false)}>
          <Pressable style={[edStyles.sheetCard, { width: '100%', maxWidth: '100%' }]} onPress={(e) => e.stopPropagation()}>
            ...
            {/* Target Baseline Source Option Item */}
            <Pressable
              style={edStyles.sheetItem}
              onPress={() => setUseRoutineTargets(prev => !prev)}
              android_ripple={rippleTokens.surface}
            >
              <Ionicons
                name={useRoutineTargets ? "document-text-outline" : "time-outline"}
                size={22}
              ...
              <View style={{ flex: 1, flexShrink: 1 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 4, marginBottom: 2 }}>
                  ...
                  <View style={{
                    backgroundColor: useRoutineTargets ? `${colors.violet || '#7C5CFC'}25` : `${colors.accent}25`,
                    borderColor: useRoutineTargets ? (colors.violet || '#7C5CFC') : colors.accent,
                    borderWidth: 1,
                    paddingHorizontal: 8,
                    paddingVertical: 2,
                    borderRadius: radius.xs,
                    ...
                    <Text style={{
                      ...
                      {useRoutineTargets ? i18n.t('routineEditor.routineBased') : i18n.t('routineEditor.historyBased')}
                    </Text>
                  </View>
                </View>
                <Text style={{ color: colors.textMuted, fontSize: font.sizes.xs, fontFamily: font.regular, lineHeight: 16 }}>
                  {useRoutineTargets ? i18n.t('routineEditor.routineBasedDesc') : i18n.t('routineEditor.historyBasedDesc')}
                </Text>
              </View>
            </Pressable>
            ...
            <Pressable style={edStyles.sheetCancel} onPress={() => setIsRoutineSettingsVisible(false)}>
              <Text style={edStyles.sheetCancelText}>{i18n.t('common.done')}</Text>
            </Pressable>
          </Pressable>
        </Pressable>
      </Modal>
