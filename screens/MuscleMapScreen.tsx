...
// ─── Images ──────────────────────────────────────────────────────────────────
// Pure SVG MuscleWiki maps are used instead of images

// The PNGs are displayed at this size; SVG overlay uses the same viewBox
const IMG_W = 220;
const IMG_H = 380;
...
function muscleColor(
  ...
  const map: Record<string, string> = {
    ...
    Calves:     colors.muscle.default,
    Abs:        colors.muscle.default,
  };
  const fill = map[muscle] ?? colors.muscle.default;
  const opacity = intensity < 0.05 ? 0.0 : Math.max(0.22, intensity * 0.85);
  return { fill, opacity };
}
...
function getMuscleStyle(
  ...
  if (isSelected) {
    return {
      fill: '#F0506E', // Vibrant electric red for selection focus
      opacity: 1.0,
    };
  }

  const intensity = getIntensity(sets, maxSets);
  if (sets === 0) {
    return {
      fill: '#151922', // Sleek AMOLED dark slate color for inactive muscles
      opacity: 0.9,
    };
  }
  ...
  const fill = map[muscle] ?? colors.muscle.default;
  const opacity = Math.max(0.4, intensity * 0.95);
  return { fill, opacity };
}

// ─── FRONT SVG MAP (MuscleWiki pure SVG structure) ───────────────────────────
const FrontSvgMap: React.FC<{
  muscleSets: Record<string, number>;
  maxSets: number;
  selectedMuscle: string | null;
  onMusclePress: (muscle: string) => void;
}> = ({ muscleSets, maxSets, selectedMuscle, onMusclePress }) => {
  const ca = getMuscleStyle('Calves',     muscleSets.Calves     ?? 0, maxSets, selectedMuscle);
  const qu = getMuscleStyle('Quads',      muscleSets.Quads      ?? 0, maxSets, selectedMuscle);
  const ab = getMuscleStyle('Abs',        muscleSets.Abs        ?? 0, maxSets, selectedMuscle);
  const sh = getMuscleStyle('Shoulders',  muscleSets.Shoulders  ?? 0, maxSets, selectedMuscle);
  const bi = getMuscleStyle('Biceps',     muscleSets.Biceps     ?? 0, maxSets, selectedMuscle);
  const ch = getMuscleStyle('Chest',      muscleSets.Chest      ?? 0, maxSets, selectedMuscle);
  const tr = getMuscleStyle('Shoulders',  muscleSets.Shoulders  ?? 0, maxSets, selectedMuscle);
  ...
  return (
    ...
    >
      ...
      {/* ── CALVES ── */}
      <G id="calves" onPress={() => onMusclePress('Calves')}>
        <Path onPress={() => onMusclePress('Calves')}
      ...
      {/* ── QUADS ── */}
      <G id="quads" onPress={() => onMusclePress('Quads')}>
        <Path onPress={() => onMusclePress('Quads')}
      ...
      {/* ── ABDOMINALS ── */}
      <G id="abdominals" onPress={() => onMusclePress('Abs')}>
        <Path onPress={() => onMusclePress('Abs')}
          ...
          fill={ab.fill}
          fillOpacity={ab.opacity}
        />
      </G>

      {/* ── OBLIQUES ── */}
      <G id="obliques" onPress={() => onMusclePress('Abs')}>
        <Path onPress={() => onMusclePress('Abs')}
      ...
      {/* ── BICEPS ── */}
      <G id="biceps" onPress={() => onMusclePress('Biceps')}>
        <Path onPress={() => onMusclePress('Biceps')}
      ...
      {/* ── SHOULDERS ── */}
      <G id="front-shoulders" onPress={() => onMusclePress('Shoulders')}>
        <Path onPress={() => onMusclePress('Shoulders')}
      ...
      {/* ── CHEST ── */}
      <G id="chest" onPress={() => onMusclePress('Chest')}>
        <Path onPress={() => onMusclePress('Chest')}
      ...
      {/* ── TRAPS ── */}
      <G id="traps" onPress={() => onMusclePress('Shoulders')}>
        <Path onPress={() => onMusclePress('Shoulders')}
      ...
      <G id="body">
        ...
        <Path d="M29,521.95v.02c1.13,7.67,5.05,13.92,10.29,16.59,4.74,2.41,10.96,2.08,17.03-.58" fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.46} />
        <Path d="
fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.46} />
        {/* ── Jaw / ear curves ── */}
        <G>
          <Path d="M370.84,135.77s-10.57,20.48-40.26,25.91" fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.46} />
          <Path d="M289.97,135.77s10.57,20.48,40.26,25.91" fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.46} />
        </G>
        {/* ── Face / skull detail ── */}
        <Path d="M282.69,83.67c-5.36-3.86-3.61-20.07-6.05-37.39-2.56-18.19,16.27-32.16,24.89-34.59,5.86-1.65-18.28,1.29,6.39-6.76s33.37,2.13,37.66,3.41c3.74-.45-9.93-8.43,11.87-1.12,20.36,6.82,23.77,22.8,22.07,39.35-2.2,21.32,2.15,29.08-1.76,37.1" fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.46} />
      </G>
    </Svg>
  );
};

// ─── BACK SVG MAP (MuscleWiki pure SVG structure) ───────────────────────────
const BackSvgMap: React.FC<{
  muscleSets: Record<string, number>;
  maxSets: number;
  selectedMuscle: string | null;
  onMusclePress: (muscle: string) => void;
  ...
  const ca = getMuscleStyle('Calves',      muscleSets.Calves      ?? 0, maxSets, selectedMuscle);
  const hm = getMuscleStyle('Hamstrings',  muscleSets.Hamstrings  ?? 0, maxSets, selectedMuscle);
  const gl = getMuscleStyle('Glutes',      muscleSets.Glutes      ?? 0, maxSets, selectedMuscle);
  const tr = getMuscleStyle('Triceps',     muscleSets.Triceps     ?? 0, maxSets, selectedMuscle);
  const la = getMuscleStyle('Back',        muscleSets.Back        ?? 0, maxSets, selectedMuscle);
  const tp = getMuscleStyle('Shoulders',   muscleSets.Shoulders   ?? 0, maxSets, selectedMuscle);
  const rd = getMuscleStyle('Rear Delts',  muscleSets['Rear Delts'] ?? 0, maxSets, selectedMuscle);
  ...
  return (
    ...
    >
      ...
      {/* ── HAMSTRINGS ── */}
      <G id="hamstrings" onPress={() => onMusclePress('Hamstrings')}>
        <Path onPress={() => onMusclePress('Hamstrings')}
      ...
      {/* ── GLUTES ── */}
      <G id="glutes" onPress={() => onMusclePress('Glutes')}>
        <Path onPress={() => onMusclePress('Glutes')}
      ...
      {/* ── TRICEPS ── */}
      <G id="triceps" onPress={() => onMusclePress('Triceps')}>
        <Path onPress={() => onMusclePress('Triceps')}
      ...
      {/* ── LATS ── */}
      <G id="lats" onPress={() => onMusclePress('Back')}>
        <Path onPress={() => onMusclePress('Back')}
      ...
      {/* ── LOWER BACK ── */}
      <G id="lowerback" onPress={() => onMusclePress('Back')}>
        <Path onPress={() => onMusclePress('Back')}
      ...
      {/* ── TRAPS MIDDLE ── */}
      <G id="traps-middle" onPress={() => onMusclePress('Back')}>
        <Path onPress={() => onMusclePress('Back')}
      ...
      {/* ── REAR SHOULDERS ── */}
      <G id="rear-shoulders" onPress={() => onMusclePress('Rear Delts')}>
        <Path onPress={() => onMusclePress('Rear Delts')}
      ...
      <G id="body">
        ...
        <Path d="M223.86,211.87c23.48-.79,35.4,13.24,40.81,19.16,12.57,13.74,20.72.94,65.59.94" fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.52} />
        <Path d="M264.67,231.03c52.5,96.35,51.77,138.65,65.59,138.65" fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.52} />
        <Path d="M146.43,292.74s7.92-6.67,16.02-7.1c8.1-.43,32.12-.43,50.22-15.45,18.1-15.01,34.72-25.92,50.42-40.9" fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.52} />
        <Path d="
fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.52} />
        </G>
        ...
        <Path d="M330.26,131.3c-15.79,0-25.13,12.03-29.48,9.51-4.36-2.51-8.08-13.01-16.42-23.77s.36-12.56-9.07-42.7c-1.49-4.74-2.14-17.9-2.17-22.01v-.04c-.04-5.66,1.11-10.73,2.93-15.18" fill="none" stroke="#484a68" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3.52} />
      </G>
    </Svg>
  );
};
...
const MUSCLE_LIST = [
  ...
  { key: 'Calves',      color: colors.muscle.default,     view: 'back'  },
  { key: 'Abs',         color: colors.muscle.default,     view: 'front' },
] as const;

type MuscleKey = typeof MUSCLE_LIST[number]['key'];
...
const MuscleMapScreen: React.FC<MuscleMapScreenProps> = ({ weeklyMuscleSets, sessions, exercisesList }) => {
  ...
  const handleMusclePress = (muscle: string) => {
    ...
    // Spring bounce animation on the SVG map container
  ...
  const filteredMuscles = useMemo(
    () => sortedMuscles.filter(m => m.view === view || m.view === 'both'),
    [sortedMuscles, view],
  );
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        ...
        {/* ── Body Image / SVG Muscle Map ── */}
        <View style={styles.bodyContainer}>
          <Animated.View style={[styles.svgCard, { transform: [{ scale: scaleAnim }] }]}>
            {view === 'front' ? (
              <View style={{ position: 'relative' }}>
                ...
                <View style={styles.touchOverlay}>
                  {/* Traps */}
                  <Pressable style={[styles.touchZone, { top: '17.5%', left: '38%', width: '24%', height: '6%' }]} onPress={() => handleMusclePress('Shoulders')} />
                  
                  {/* Chest */}
                  <Pressable style={[styles.touchZone, { top: '23%', left: '32%', width: '36%', height: '9%' }]} onPress={() => handleMusclePress('Chest')} />
                  
                  {/* Shoulders */}
                  <Pressable style={[styles.touchZone, { top: '21%', left: '20%', width: '15%', height: '9%' }]} onPress={() => handleMusclePress('Shoulders')} />
                  <Pressable style={[styles.touchZone, { top: '21%', left: '65%', width: '15%', height: '9%' }]} onPress={() => handleMusclePress('Shoulders')} />
                  
                  {/* Biceps */}
                  <Pressable style={[styles.touchZone, { top: '29%', left: '16%', width: '14%', height: '10%' }]} onPress={() => handleMusclePress('Biceps')} />
                  <Pressable style={[styles.touchZone, { top: '29%', left: '70%', width: '14%', height: '10%' }]} onPress={() => handleMusclePress('Biceps')} />
                  
                  {/* Abs / Obliques */}
                  <Pressable style={[styles.touchZone, { top: '32%', left: '34%', width: '32%', height: '16%' }]} onPress={() => handleMusclePress('Abs')} />
                  
                  {/* Quads */}
                  <Pressable style={[styles.touchZone, { top: '47%', left: '26%', width: '22%', height: '22%' }]} onPress={() => handleMusclePress('Quads')} />
                  <Pressable style={[styles.touchZone, { top: '47%', left: '52%', width: '22%', height: '22%' }]} onPress={() => handleMusclePress('Quads')} />
                  
                  {/* Calves */}
                  <Pressable style={[styles.touchZone, { top: '72%', left: '26%', width: '18%', height: '22%' }]} onPress={() => handleMusclePress('Calves')} />
                  <Pressable style={[styles.touchZone, { top: '72%', left: '56%', width: '18%', height: '22%' }]} onPress={() => handleMusclePress('Calves')} />
                </View>
              </View>
            ) : (
              <View style={{ position: 'relative' }}>
                ...
                <View style={styles.touchOverlay}>
                  {/* Traps */}
                  <Pressable style={[styles.touchZone, { top: '17%', left: '38%', width: '24%', height: '10%' }]} onPress={() => handleMusclePress('Shoulders')} />
                  
                  {/* Rear Delts */}
                  <Pressable style={[styles.touchZone, { top: '21%', left: '20%', width: '15%', height: '9%' }]} onPress={() => handleMusclePress('Rear Delts')} />
                  <Pressable style={[styles.touchZone, { top: '21%', left: '65%', width: '15%', height: '9%' }]} onPress={() => handleMusclePress('Rear Delts')} />
                  
                  {/* Triceps */}
                  <Pressable style={[styles.touchZone, { top: '28%', left: '16%', width: '14%', height: '11%' }]} onPress={() => handleMusclePress('Triceps')} />
                  <Pressable style={[styles.touchZone, { top: '28%', left: '70%', width: '14%', height: '11%' }]} onPress={() => handleMusclePress('Triceps')} />
                  
                  {/* Back / Lats */}
                  <Pressable style={[styles.touchZone, { top: '30%', left: '30%', width: '40%', height: '18%' }]} onPress={() => handleMusclePress('Back')} />
                  
                  {/* Glutes */}
                  <Pressable style={[styles.touchZone, { top: '46%', left: '30%', width: '40%', height: '11%' }]} onPress={() => handleMusclePress('Glutes')} />
                  
                  {/* Hamstrings */}
                  <Pressable style={[styles.touchZone, { top: '56%', left: '26%', width: '22%', height: '22%' }]} onPress={() => handleMusclePress('Hamstrings')} />
                  <Pressable style={[styles.touchZone, { top: '56%', left: '52%', width: '22%', height: '22%' }]} onPress={() => handleMusclePress('Hamstrings')} />
                  
                  {/* Calves */}
                  <Pressable style={[styles.touchZone, { top: '76%', left: '26%', width: '18%', height: '20%' }]} onPress={() => handleMusclePress('Calves')} />
                  <Pressable style={[styles.touchZone, { top: '76%', left: '56%', width: '18%', height: '20%' }]} onPress={() => handleMusclePress('Calves')} />
                </View>
              </View>
            )}
          </Animated.View>
          ...
          {filteredMuscles.map(muscle => {
            ...
            return (
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.legendItem,
                  pressed && { opacity: 0.6, backgroundColor: colors.surface2 }
                ]}
              ...
              >
                <View
                  style={[
                    styles.legendDot,
                    { backgroundColor: muscle.color, opacity: Math.max(0.25, intensity) },
                  ]}
                />
      ...
      {selectedMuscle !== null && (
        <Animated.View style={styles.bottomSheetContainer}>
          ...
          >
            ...
            <Text style={styles.sheetSectionTitle}>THIS WEEK'S WORKOUTS</Text>
            {weeklyExercisesForMuscle.length === 0 ? (
              <Text style={styles.sheetEmptyText}>No exercises logged for {selectedMuscle} this week.</Text>
            ) : (
              weeklyExercisesForMuscle.map((ex, idx) => (
                <View key={idx} style={styles.sheetExerciseRow}>
                  <Text style={styles.sheetExerciseName}>{ex.name}</Text>
                  <Text style={styles.sheetExerciseDetails}>{ex.sets} sets • {ex.weight} kg max</Text>
                </View>
              ))
            )}

            {/* Section 2: Suggestion Library */}
            <Text style={[styles.sheetSectionTitle, { marginTop: spacing.md }]}>SUGGESTED EXERCISES</Text>
              ...
              {suggestedExercisesForMuscle.length === 0 ? (
                ...
                suggestedExercisesForMuscle.map((ex, idx) => (
                  <View key={idx} style={styles.suggestionChip}>
                    <Text style={styles.suggestionChipText}>{ex.name}</Text>
                  </View>
                ))
              )}
            </View>
