...
const FrontSvgMap: React.FC<{
  ...
  const ca = getMuscleStyle('Calves',     muscleSets.Calves     ?? 0, maxSets);
  const qu = getMuscleStyle('Quads',      muscleSets.Quads      ?? 0, maxSets);
  const ab = getMuscleStyle('Abs',        muscleSets.Abs        ?? 0, maxSets);
  const sh = getMuscleStyle('Shoulders',  muscleSets.Shoulders  ?? 0, maxSets);
  const bi = getMuscleStyle('Biceps',     muscleSets.Biceps     ?? 0, maxSets);
  const ch = getMuscleStyle('Chest',      muscleSets.Chest      ?? 0, maxSets);
  const tr = getMuscleStyle('Shoulders',  muscleSets.Shoulders  ?? 0, maxSets);
...
const BackSvgMap: React.FC<{
  ...
  const ca = getMuscleStyle('Calves',      muscleSets.Calves      ?? 0, maxSets);
  const hm = getMuscleStyle('Hamstrings',  muscleSets.Hamstrings  ?? 0, maxSets);
  const gl = getMuscleStyle('Glutes',      muscleSets.Glutes      ?? 0, maxSets);
  const tr = getMuscleStyle('Triceps',     muscleSets.Triceps     ?? 0, maxSets);
  const la = getMuscleStyle('Back',        muscleSets.Back        ?? 0, maxSets);
  const tp = getMuscleStyle('Shoulders',   muscleSets.Shoulders   ?? 0, maxSets);
  const rd = getMuscleStyle('Rear Delts',  muscleSets['Rear Delts'] ?? 0, maxSets);
      ...
      >
        ...
        <View style={styles.bodyContainer}>
          <View style={styles.svgCard}>
            {view === 'front' ? (
              // Pure SVG MuscleWiki-style map for the front view
              <FrontSvgMap muscleSets={weeklyMuscleSets} maxSets={maxSets} />
            ) : (
              // Pure SVG MuscleWiki-style map for the back view
              <BackSvgMap muscleSets={weeklyMuscleSets} maxSets={maxSets} />
            )}
          </View>
